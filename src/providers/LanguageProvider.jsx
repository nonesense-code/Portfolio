...
export const LanguageProvider = ({ children }) => {
  ...
  useEffect(() => {
    const defaultLanguage =
      allLanguages.find((language) => language.default) || allLanguages[0];
    setDefaultLanguageId(defaultLanguage.id);

    const localStorageItem = window.localStorage.getItem(localStorageName);
    const savedLanguage = allLanguages.find(
      (language) => language.id === localStorageItem
    );
    if (savedLanguage) {
