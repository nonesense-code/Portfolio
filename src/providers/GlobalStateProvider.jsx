...
export const GlobalStateProvider = ({ children }) => {
  ...
  const _onSectionChange = (section) => {
    if (window.targetSectionId === section.id) return;
  ...
  const setActiveSectionFromCategory = (categoryId) => {
    if (isCategoryActive(categoryId)) return;

    const category = categories.find((category) => category.id === categoryId);
    const sections = getCategorySections(category);

    if (!category || !sections || sections.length === 0) return;

    const targetSectionId =
      window.visitHistory && window.visitHistory[categoryId]
  ...
  const isCategoryActive = (categoryId) => {
    ...
    if (!displayingSection || !displayingSection.category) return false;
